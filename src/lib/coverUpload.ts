 ...
 *
 * Здесь добавлен второй способ: админ выбирает файл, тот приводится к
 * WebP ≤800 px и уходит в публичный бакет Storage `covers`, а в
 * `titles.cover_url` записывается публичный URL Storage. CSP `img-src`
 * пропускает `*.supabase.co` (vercel.json), а `isMediaUrlCspAllowed` —
 * подтверждает это до сохранения, поэтому форма принимает оба вида путей.
 *
...
/** То же самое расширениями — для подсказки и для дропзоны. */
export const COVER_ACCEPT_EXT = '.jpg,.jpeg,.png,.webp';

/**
 * Сигнатуры (magic bytes), которые считаем обложкой. HEIC/TIFF/PDF/GIF
 * отклоняются ДО загрузки: страницы глав их перекодируют (preparePageImage),
 * а для обложки «почти рабочий» экзотический формат дороже понятной ошибки —
 * битая картинка в каталоге видна всем читателям.
 */
const COVER_SIGNATURES: readonly string[] = ['jpg', 'png', 'webp'];
...
export async function validateCoverFile(file: File | null | undefined): Promise<CoverValidation> {
  ...
  if (!ext || !COVER_SIGNATURES.includes(ext)) {
    return {
      ok: false,
      reason: `Формат файла \`${file.name}\` не поддерживается. Допустимы JPEG, PNG или WebP.`,
    };
  }
...
/**
 * Имя объекта в бакете `covers`: `{slug}-{time36}-{rand8}.{ext}`.
 *
...
export interface PreparedCover {
  /** Что реально полетит в Storage: WebP ≤800 px либо (без canvas) исходник. */
  blob: Blob;
  /** Удалось ли пережать в WebP. `false` — браузер без WebP-энкодера. */
  compressed: boolean;
}

/**
 * Привести файл к тому, что кладётся в бакет: WebP шириной ≤800 px.
 *
...
export async function prepareCoverImage(file: File): Promise<PreparedCover> {
  ...
  try {
    ...
    if (webp.size > 0 && webp.type === 'image/webp') {
      return { blob: webp, compressed: true };
    }
