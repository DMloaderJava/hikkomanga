...
2. **Путь в БД совпадает с именем файла?** Канонический путь
   регистрозависим и начинается с `/`. Форма (TitleForm → «Обложка тайтла»)
   сама приводит частые относительные записи к канону —
   `media/covers/x.webp`, `./media/covers/x.webp`, `public/media/covers/x.webp`
   → `/media/covers/x.webp` (единая точка — `normalizeCoverUrl`; data-слой
   применяет те же правила при чтении и записи, так что «голые» пути,
   внесённые руками в Table Editor, чинятся автоматически). Перед
   сохранением форма ещё проверяет HEAD-запросом, что файл существует,
   и показывает предупреждение (не блокирует). Внешние `http(s)://` URL
   не поддерживаются: `http://` блокирует сам браузер (смешанный контент),
   внешний `https://` — политикой проекта: мёртвый домен или
   хотлинк-защита превратит каталог в стену битых картинок.
