...
11. `00000000000019_title_covers.sql` — публичный бакет обложек `covers`
    + политики (чтение всем, запись — роль admin). Историческая: имя бакета
    заменено в следующей миграции.
12. `00000000000020_title_covers_bucket.sql` — публичный бакет `title-covers`
    (то же имя, что `COVER_BUCKET` в `src/data/storage.ts`), `allowed_mime_types
    = {image/jpeg, image/png, image/webp, image/gif}` и `file_size_limit = 5 MB`.
    Сюда админка кладёт обложки, загруженные в TitleForm кнопкой «Загрузить
    файл». Пустой бакет `covers` миграция убирает. Без неё загрузка обложки
    падает с 400 «Bucket not found» → «Бакет `title-covers` не создан».
...
Обложка тайтла бывает двух видов, форма в админке (TitleForm → «Обложка тайтла»)
поддерживает оба:

1. **Файл репозитория** — `public/media/covers/{slug}.webp`, в `titles.cover_url`
   относительный путь `/media/covers/{slug}.webp`. Штатно для сидов и «вечных»
   картинок: файл в git, кэш `'self'`, вес каталога ограничен бюджетом
   (`scripts/check-budgets.mjs`).
2. **Загрузка из админки** — кнопка «Загрузить файл» (или перетаскивание файла
   в превью): JPEG, PNG, WebP или GIF до 5 MB (`accept="image/*"`, настоящий
   фильтр формата — по байтам файла). Клиент жмёт картинку в WebP ≤800 px,
   GIF уходит как есть — с анимацией (`src/lib/coverUpload.ts`), и кладёт её в
   публичный бакет `title-covers` (миграция
   `00000000000020_title_covers_bucket.sql`), в `titles.cover_url` — публичный
   URL `…/storage/v1/object/public/title-covers/{slug}-{time}-{rand}.{webp|jpg|png|gif}`.
   Без настроенного Supabase (демо/превью) вместо Storage получается data-URL —
   флоу тот же, бэкенд не нужен.

Рендер обложки — только `src/components/manga/CoverImage.tsx`: пустой/битый путь
→ статический плейсхолдер `/media/placeholder-cover.svg` + `data-cover-error` в
DOM (в dev — ещё и `console.warn('[cover] failed …')`, в прод — счётчик
`cover_error` в Vercel Analytics). Если видите плейсхолдер — идите по цепочке:

1. **Что в `cover_url`?** Относительный путь → это файл репозитория (п. 2),
   `https://<ref>.supabase.co/storage/v1/object/public/title-covers/…` →
   загруженная обложка (п. 3; ссылки со старым `…/public/covers/…` — тоже
   загруженные, просто до миграции 20), data-URL → демо-режим без Supabase.
2. **Файл существует?** Проверьте наличие `public/media/covers/{имя}.webp`
   локально и в git. Сид-обложки генерируются разово:
   `npm i --no-save sharp && node scripts/generate-seed-covers.mjs`
   (тот же визуальный генератор, что в демо-режиме, WebP 800px). Путь
   регистрозависим и должен начинаться с `/`; меняется в админке или руками в
   Table Editor. Внешние `http(s)://` URL (не Storage) форма отклоняет, а
   `normalizeMediaUrl` возвращает относительные пути как есть.
3. **Загруженная обложка?** Откройте URL из `cover_url` напрямую: 404 — объект
   удалён или не докачался, пересохраните обложку в TitleForm. Ошибка при самой
   загрузке («Бакет `title-covers` не создан» / «Нет прав на загрузку обложек») —
   не накатана миграция 20 или у пользователя нет роли admin. Отдельный случай:
   запрос уходит в `/object/covers/…` и ловит 400 «Bucket not found» — значит
   бандл собран до переименования бакета, пересоберите/задеплойте сайт.
4. **og:image абсолютный в пререндере?** `src/lib/seo.ts` собирает
...
Проверить ВСЕ тайтлы живой базы одним скриптом — он сверяет файлы в `public/`
и делает HEAD-запрос к обложкам из бакета `title-covers`:

```bash
npm run check:covers
# Exit: 0 — все обложки доступны; 1 — есть битые пути/файлы/URL;
#       2 — сама БД недоступна (переменные/сеть).
```

Вес каталога обложек в репозитории ограничен бюджетом 500 kB
(`scripts/check-budgets.mjs`, входит в `npm run build`): новый файл в
`public/media/covers/` — это осознанный коммит. Загруженные через админку
обложки живут в Storage и бюджет сборки не расходуют. CSP `img-src` покрывает
оба вида: `'self'` для файлов репозитория и `*.supabase.co` для Storage.

#### Страницы, озвучки и загруженные обложки (Supabase Storage)

Для `pages.image_url`, `chapter_voiceovers.audio_url` и загруженных из админки
обложек (`titles.cover_url` → бакет `title-covers`) актуально прежнее:

1. **Бакеты.** `manga` (миграция `00000000000002_storage_buckets.sql`) и
   `title-covers` (миграция `00000000000020_title_covers_bucket.sql`) — public,
   `voiceovers` и `hikko-originals` — приватные. Приватный `manga` = 400/403
   на каждую страницу главы, приватный `title-covers` = плейсхолдер вместо
   обложек в каталоге. У `title-covers` заданы `allowed_mime_types`
   (jpeg/png/webp/gif) и `file_size_limit = 5 MB` — если бакет создавали руками
   с другим списком, JPEG/PNG/GIF упрутся в 400: перечитайте миграцию 20.
...
| Черновик создан, обложка не видна | Обложка заявки лежит в бакете `submissions` (URL в payload.cover_url). Откройте тайтл в админке и нажмите «Загрузить файл» — обложка уйдёт в бакет `title-covers` (или перенесите файл в `public/media/covers/{slug}.webp` и укажите путь). См. раздел «Обложки». |
...
| «Чат недоступен: edge-функция chat не ответила» | Функция `chat` не задеплоена (404) или в проекте нет ни одного сохранённого Gemini-ключа. Проверка: `npm run check:supabase` (раздел Edge Functions). В **демо-режиме** (`npm run dev` без `.env`) этой ошибки быть не должно: панель показывает баннер «Демо-режим: Supabase не настроен» — если видите текст про деплой без баннера, значит `VITE_SUPABASE_*` заданы, но сборка смотрит в другой проект |
