...
Единая точка работы с URL медиа — `src/lib/storageUrl.ts` (нормализация,
сборка public-URL, извлечение пути бакета, проверка CSP). Рендер обложки —
только `src/components/manga/CoverImage.tsx`: пустой/битый URL → статический
...
1. **CSP.** `vercel.json` → `img-src`: `'self' data: blob: https://*.supabase.co
   https://*.supabase.in`. Внешние CDN и прямые ссылки на чужие хостинги
   браузер молча не загрузит. Форма тайтла отклоняет такие URL при сохранении —
   если обложка попала в базу раньше проверки, перезалейте файл через админку.
2. **Бакет.** Storage → Buckets: `manga` должен быть **public**
   (миграция `00000000000002_storage_buckets.sql`). Приватный `manga` =
   400/403 на каждый URL.
3. **RLS на `storage.objects`.** После пересоздания проекта политики бакетов
   не появляются сами — проверьте `pg_policies` для `storage.objects`
   (SQL в разделе «Если проект Supabase пересоздавали или удаляли»).
4. **Мёртвый ref.** URL указывает на удалённый проект → `src/lib/storageUrl.ts`,
   список `LEGACY_SUPABASE_REFS` (переадресация на текущий ref) — и убедитесь,
   что файл реально перенесён в новый проект, иначе 404.
5. **Кэш CDN.** Каждая загрузка обложки пишет файл под **новым** путём
   (`covers/<timestamp>-<rand>.webp`), поэтому старый URL из кэша не вернётся.
   Если меняете файл с тем же именем руками (SQL/дашборд) — CDN продолжит
   отдавать старую версию до истечения `cacheControl` (тут — год).
6. **Http, а не https.** Нормализация `normalizeMediaUrl` повышает `http://`
   до `https://` на чтении и записи, но строки, записанные до её появления,
   могло не коснуться — проверьте `titles.cover_url` в Table Editor.

Диагностика одним скриптом — по всем тайтлам живой базы:

```bash
npm run check:covers            # таблица slug | тип | статус | content-type | размер
npm run check:covers -- --pages # + страницы глав (pages.image_url)
```

Exit-коды: `0` — всё доступно; `1` — есть битые/внешние URL; `2` — проверка не
состоялась (переменные не заданы или база/сеть недоступны целиком — среда, не
данные). Тот же шаг в сокращённом виде входит в `npm run check:supabase`
(раздел «8. Публичные медиа»). Бюджеты веса сборки проверяет
`npm run check:budgets` (входит в `npm run build`): initial JS ≤ 140 kB gzip,
локальные `/media` первого экрана каталога ≤ 300 kB.

Сжатие обложек уже встроено: при загрузке через админку файл уходит в бакет
как WebP ≤ 800 px (`src/lib/imageCompress.ts`). Опциональные серверные
миниатюры (srcset 400w/800w через Supabase Image Transformation) включаются
`VITE_COVER_TRANSFORMS=1` при сборке — только на плане с включёнными
трансформациями; без них `CoverImage` сам откатывается на оригинальный URL.
