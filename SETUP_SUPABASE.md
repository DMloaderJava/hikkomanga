...
11. `00000000000018_title_covers.sql` — публичный бакет `covers` + политики
    (чтение всем, запись — роль admin): сюда админка кладёт обложки,
    загруженные в TitleForm кнопкой «Загрузить файл». Без неё загрузка
    обложки падает с «Бакет `covers` не создан».
...
2. **Загрузка из админки** — кнопка «Загрузить файл» (или перетаскивание файла
   в превью): JPEG/PNG/WebP до 5 MB, клиент жмёт в WebP ≤800 px
   (`src/lib/coverUpload.ts`) и кладёт в публичный бакет `covers`
   (миграция `00000000000018_title_covers.sql`), в `titles.cover_url` —
...
1. **Что в `cover_url`?** Относительный путь → это файл репозитория (п. 2),
   `https://<ref>.supabase.co/storage/v1/object/public/covers/…` → загруженная
   обложка (п. 3), data-URL → демо-режим без Supabase.
...
3. **Загруженная обложка?** Откройте URL из `cover_url` напрямую: 404 — объект
   ...
   загрузке («Бакет `covers` не создан» / «Нет прав на загрузку обложек») —
   не накатана миграция 18 или у пользователя нет роли admin.
...
и делает HEAD-запрос к обложкам из бакета `covers`:
...
обложек (`titles.cover_url` → бакет `covers`) актуально прежнее:

1. **Бакеты.** `manga` (миграция `00000000000002_storage_buckets.sql`) и
   `covers` (миграция `00000000000018_title_covers.sql`) — public,
   `voiceovers` и `hikko-originals` — приватные. Приватный `manga` = 400/403
   на каждую страницу главы, приватный `covers` = плейсхолдер вместо обложек
   в каталоге.
...
| Черновик создан, обложка не видна | Обложка заявки лежит в бакете `submissions` (URL в payload.cover_url). Откройте тайтл в админке и нажмите «Загрузить файл» — обложка уйдёт в бакет `covers` (или перенесите файл в `public/media/covers/{slug}.webp` и укажите путь). См. раздел «Обложки». |
