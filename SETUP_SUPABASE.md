...
В демо-режиме (`import.meta.env.DEV` или когда Supabase недоступен по сети)
`src/data/auth.ts` пускает с локальными учетками
`admin@hikkomanga.local` / `demo@hikkomanga.local`. В продакшене этот fallback
отключён, но сами пароли в коде есть — для продакшена их лучше вычистить.
