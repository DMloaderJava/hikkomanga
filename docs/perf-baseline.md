...
> **Рабочая ветка:** `arena/01a0afe7-hikkomanga` (сессионная ветка, указатель `feature/perf-reader-baseline`)  
...
- **TTFB / HTML:** ~15.5 ms (Status 200, 28.8 kB HTML с пререндером)
...
- **Время выборки данных главы (Cold Data Fetch):** ~0.25 ms (in-memory mockStore).
- **Время выборки данных следующей главы (Uncached Navigation):** ~0.20 ms.
